...
import Dialer from '../../components/real-time-pollutant-page/pm10/Dialer';
import Component1 from '../../components/real-time-pollutant-page/pm10/component1';
import Component3 from '../../components/real-time-pollutant-page/pm10/component3';
import Component4 from '../../components/real-time-pollutant-page/pm10/component4';
import Component5 from '../../components/real-time-pollutant-page/pm10/component5';
import HistoricalChart from '../../components/real-time-pollutant-page/pm10/HistoricalChart';
import HistoricalHistogram from '../../components/real-time-pollutant-page/pm10/HistoricalHistogram';
...
function PM10Page() {
  ...
  const initialPM10Value =
    location.state?.pollutantValue || location.state?.pollutantData?.pm10 || null;
  ...
  // Get PM2.5 category based on PM2.5 value (used here to determine a color/style in the Dialer)
  const getPM25Category = (pm25) => {
    ...
    return pm25Categories[pm25Categories.length - 1];
  };

  // Helper function to fetch detailed data for a station (if needed)
  ...
  // When a station is selected, only fetch historical data.
  useEffect(() => {
    if (selectedStation) {
  ...
  // Remove fetching current PM10 data from API.
  // The current PM10 value and pollutantData are taken from the passed state.
  // We still fetch historical data.
  ...
  // Refresh now only updates historical data and the lastUpdated time.
  const handleRefresh = () => {
    if (selectedStation) {
  ...
  const pm25Category = pm10Value ? getPM25Category(pm10Value) : null;
  ...
  // Chart options for historical display
  const chartOptions = {
    responsive: true,
    ...
    <div className="app">
      ...
      {selectedStation && (
        <div className="dashboard">
          {/* Pass pollutantData to StationInfo */}
          ...
          <div style={{ display: 'flex', width: '100%' }}>
            <div style={{ flex: '0 0 70%' }}>
              <Dialer currentAQI={pm10Value} aqiCategory={pm25Category} />
            </div>
            <div style={{ flex: '0 0 30%', marginLeft: '10px', paddingRight: '10px' }}>
              <MapView selectedStation={selectedStation} currentAQI={null} pm25Value={pm10Value} />
            </div>
          </div>
          ...
          <div style={{ marginTop: '20px' }}>
            <Component1 pm25Value={pm10Value} />
          </div>
      ...
      {!selectedStation && (
        <div className="no-station">
          <p>Please select a station to view PM10 data.</p>
        </div>
      )}
