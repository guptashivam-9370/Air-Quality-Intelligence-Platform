...
const Component1 = ({ pm25Value }) => {
  ...
  const getHealthRiskLevel = (value) => {
    if (value <= 8330) return { level: "Good", color: "#4CAF50" };
    if (value <= 16670) return { level: "Moderate", color: "#FFC107" };
    if (value <= 25000) return { level: "Poor", color: "#FF9800" };
    if (value <= 33330) return { level: "Unhealthy", color: "#F44336" };
    if (value <= 41670) return { level: "Severe", color: "#9C27B0" };
  ...
  const getStatusMessage = (value) => {
    if (value <= 8330)
      return "which is within safe CO limits, but sensitive individuals should be cautious.";
    if (value <= 16670)
      return "indicating moderate CO levels; sensitive groups should reduce outdoor activities.";
    if (value <= 25000)
      return "indicating poor CO levels; everyone should take precautions.";
    if (value <= 33330)
      return "indicating unhealthy CO levels; avoid outdoor activities.";
    if (value <= 41670)
      return "indicating very unhealthy CO levels; everyone should avoid outdoor exposure.";
    return "indicating hazardous CO levels; stay indoors if possible.";
  };
  ...
  return (
    <div className="pm25-containers">
      {/* Header section */}
      <h1 className="pm25-title">What is the Current CO Level?</h1>
        ...
        <div className="info-container">
          ...
          <div className="who-comparison">
            ...
            <div className="who-info">
              <p>
                The current CO level here is {timesAboveWHO}x Above the recommended WHO guideline of{' '}
