...
const Dialer = ({ currentAQI, aqiCategory }) => {
  ...
  // Function to generate background gradient based on SO₂ value
  const getBackgroundGradient = (so2) => {
    if (so2 === null) return 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)';
    if (so2 <= 40)    return 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)'; // Good
    if (so2 <= 80)    return 'linear-gradient(135deg, #f6d365 0%, #fda085 100%)'; // Moderate
    if (so2 <= 380)   return 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)'; // Poor
    if (so2 <= 800)   return 'linear-gradient(135deg, #ff0844 0%, #ffb199 100%)'; // Unhealthy
    if (so2 <= 1600)  return 'linear-gradient(135deg, #6a11cb 0%, #fc2d7f 100%)'; // Severe
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div
          ...
          style={{
            background: `conic-gradient(
              ${aqiCategory?.color || '#e0e0e0'} ${(animatedAQI / 2600) * 360}deg,
              #e0e0e0 ${(animatedAQI / 2600) * 360}deg 360deg
            )`,
        ...
        >
          ...
          <div style={{
            ...
            transform: `translateX(-50%) rotate(${(animatedAQI / 2600) * 360}deg)`,
            zIndex: 2,
            transition: 'transform 0.3s ease'
          }}>
            <div style={{
        ...
        <div className="aqi-category" style={{ textAlign: 'center', marginBottom: '20px', padding: '0 15px' }}>
          {aqiCategory && (
            <>
              <h3 style={{
                color: currentAQI > 1600 ? '#fff' : aqiCategory.color,
                fontSize: '1.5rem',
                margin: '0 0 10px',
      ...
      <div className="aqi-container" style={{
        ...
        <div className="aqi-background" style={{ position: 'relative' }}>
          ...
          <div className="category-labels" style={{
            ...
            {[
              { label: 'Good',      start: 0,    end: 40   },
              { label: 'Moderate',  start: 40,   end: 80   },
              { label: 'Poor',      start: 80,   end: 380  },
              { label: 'Unhealthy', start: 380,  end: 800  },
              { label: 'Severe',    start: 800,  end: 1600 },
              { label: 'Hazardous', start: 1600, end: 2600 }
          ...
          <div className="gradient-bar" style={{
            ...
            <div className="indicator" style={{
              ...
              borderRadius: '6px',
              top: '-2px',
              left: `${Math.min((animatedAQI / 2600) * 100, 100)}%`,
              transform: 'translateX(-50%)',
          ...
          <div className="value-markers" style={{
            ...
            {[0, 40, 80, 380, 800, 1600, 2600].map((val, i) => (
              <div key={i} style={{
                position: 'absolute',
                left: `${(val / 2600) * 100}%`,
                transform: 'translateX(-50%)',
